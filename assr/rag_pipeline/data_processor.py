...
class MitreAttackProcessor(DataPreprocessor):
    ...
    def process_mitre_dataset(self, csv_path: str) -> List[Dict[str, Any]]:
        try:
            # Use pandas to properly parse CSV file
            logging.info(f"Processing MITRE dataset from: {csv_path}")
            
            # Read CSV with proper handling of quotes and multiline values
            ...
            for idx, row in df.iterrows():
                try:
                    ...
                    # Create content for embedding - combine payload and description
                    main_content = f"MITRE {mitre_id}: {payload}"
                    if description:
                        main_content += f" Description: {description}"
                    if attack_type:
                        main_content += f" Attack Type: {attack_type}"
                    
                    # Chunk long content if needed
                    ...
                    logging.warning(f"Error processing MITRE row {idx}: {e}")
                    continue
            
            logging.info(f"Successfully processed {len(processed_data)} entries from MITRE dataset")
...
class PayloadProcessor(DataPreprocessor):
    ...
    def process_payload_dataset(self, csv_path: str) -> List[Dict[str, Any]]:
        try:
            # Use pandas to properly parse CSV file
            logging.info(f"Processing payload dataset from: {csv_path}")
            ...
            for idx, row in df.iterrows():
                try:
                    ...
                    # Create content for embedding - combine payload and description
                    main_content = f"Payload: {payload}"
                    if description:
                        main_content += f" Description: {description}"
                    if attack_type:
                        main_content += f" Attack Type: {attack_type}"
                    ...
                    logging.warning(f"Error processing payload row {idx}: {e}")
                    continue
            
            logging.info(f"Successfully processed {len(processed_data)} entries from payload dataset")
            return processed_data
